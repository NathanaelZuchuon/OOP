...
def msg_transfer(index, new_data):
	...
	new_msg.append({new_data[0]: (new_data[1], new_data[2])})
...
def return_costs(file):
	...
	return f"Le total des frais s'élève à {currentie(sep(value))} ."
...
def modify_file(file, index, value, instance=None, new_data=None):
	...
	if value == 'modification':
		users_data.insert(index, f"dict(name='{instance.user}', password='{instance.code}', "
								+f"amount={instance.amount}, limit={instance.limit}, "
